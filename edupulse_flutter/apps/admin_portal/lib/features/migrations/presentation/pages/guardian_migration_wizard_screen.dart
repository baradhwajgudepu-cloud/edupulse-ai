...
class GuardianMigrationWizardScreen extends ConsumerStatefulWidget {
  final String? jobId;

  const GuardianMigrationWizardScreen({super.key, this.jobId});

  @override
  ConsumerState<GuardianMigrationWizardScreen> createState() => _GuardianMigrationWizardScreenState();
}

class _GuardianMigrationWizardScreenState extends ConsumerState<GuardianMigrationWizardScreen> {
  int _rowPage = 0;
  ...
  void initState() {
    ...
    Future.microtask(() {
      if (widget.jobId != null) {
        ref.read(guardianMigrationWizardProvider.notifier).preloadJob(widget.jobId!);
      } else {
        final wizardState = ref.read(guardianMigrationWizardProvider);
        if (wizardState.currentStep == 0 && wizardState.selectedSchoolId == null) {
          final schoolId = ref.read(selectedSchoolIdProvider);
          if (schoolId != null) {
            ref.read(guardianMigrationWizardProvider.notifier).updateContext(schoolId);
          }
        }
      }
    });
  ...
  void _downloadTemplate() {
    const templateHeaders =
        'guardian_id,guardian_type,first_name,last_name,gender,date_of_birth,mobile,middle_name,email,aadhaar_number,occupation,income,education';
    const templateRow =
        '\nG-001,FATHER,Rajesh,Sharma,MALE,1980-05-15,9876543210,Kumar,rajesh.sharma@email.com,123456789012,Engineer,1200000,Graduate';
    downloadCsvFile('guardian_migration_template.csv', '$templateHeaders$templateRow');
  }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(guardianMigrationWizardProvider);
    final theme = Theme.of(context);

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.jobId == null ? 'New Guardian Migration' : 'Migration Job Details'),
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
          onPressed: () {
            if (state.currentStep > 0 && widget.jobId == null) {
              ref.read(guardianMigrationWizardProvider.notifier).prevStep();
            } else {
  ...
  Widget _buildStepContent(GuardianMigrationWizardState state, ThemeData theme) {
  ...
  Widget _buildContextStep(GuardianMigrationWizardState state, ThemeData theme) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text('Choose the destination campus context for the imported guardian records.'),
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (val) {
            if (val != null) {
              ref.read(guardianMigrationWizardProvider.notifier).updateContext(val);
            }
          },
        ),
        ...
        Align(
          ...
          child: ElevatedButton(
            onPressed: state.selectedSchoolId != null
                ? () => ref.read(guardianMigrationWizardProvider.notifier).nextStep()
                : null,
  ...
  Widget _buildCsvUploadStep(GuardianMigrationWizardState state, ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Upload Guardian Spreadsheet', style: theme.textTheme.headlineSmall),
        const SizedBox(height: 8),
        ...
        InkWell(
          onTap: () async {
            ...
            if (result != null && result.files.isNotEmpty) {
              ...
              if (file.bytes != null) {
                ref.read(guardianMigrationWizardProvider.notifier).updateSelectedFile(file.name, file.bytes!);
              }
            }
          },
          child: Container(
              ...
              child: state.fileName != null
                  ...
                  : const Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text('Click to select Guardian spreadsheet file'),
                      ],
                    ),
            ),
          ),
        ...
        Card(
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Format Guidelines:', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                const Text('• Mandatory Headers: guardian_id, guardian_type, first_name, last_name, gender, date_of_birth, mobile'),
                const SizedBox(height: 4),
                const Text('• Optional Headers: middle_name, email, aadhaar_number, occupation, income, education'),
                const SizedBox(height: 4),
                const Text('• Gender values: MALE, FEMALE, or OTHER'),
                const SizedBox(height: 4),
                const Text('• Guardian type: FATHER, MOTHER, or GUARDIAN'),
                const SizedBox(height: 4),
        ...
        Row(
          ...
          children: [
            OutlinedButton(
              onPressed: () => ref.read(guardianMigrationWizardProvider.notifier).prevStep(),
            ...
            ElevatedButton(
              onPressed: state.fileName != null
                  ? () async {
                      final jobCreated =
                          await ref.read(guardianMigrationWizardProvider.notifier).createImportJob();
                      if (jobCreated) {
                        final validated =
                            await ref.read(guardianMigrationWizardProvider.notifier).validateCsvFile();
                        if (validated) {
                          ref.read(guardianMigrationWizardProvider.notifier).nextStep();
                        }
                      }
  ...
  Widget _buildValidationStep(GuardianMigrationWizardState state, ThemeData theme) {
    ...
    final selectedSheet = state.selectedSheet;
          ...
          DropdownButton<String>(
            ...
            onChanged: (newSheet) {
              if (newSheet != null && newSheet != selectedSheet) {
                ref.read(guardianMigrationWizardProvider.notifier).updateSelectedSheet(newSheet);
              }
            },
          ),
          const SizedBox(height: 24),
            ...
            Row(
              children: [
                if (failedRowsCount > 0) ...[
                  OutlinedButton(
                    onPressed: () => ref.read(guardianMigrationWizardProvider.notifier).nextStep(),
                ...
                ElevatedButton(
                  onPressed: validRowsCount > 0
                      ? () {
                          if (failedRowsCount > 0) {
                            ref.read(guardianMigrationWizardProvider.notifier).nextStep(); // to review
                          } else {
                            ref.read(guardianMigrationWizardProvider.notifier).nextStep(); // review
                            ref.read(guardianMigrationWizardProvider.notifier).nextStep(); // to execution
                          }
                        }
                      : null,
  ...
  Widget _buildErrorReviewStep(GuardianMigrationWizardState state, ThemeData theme) {
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              columns: const [
                ...
                DataColumn(label: Text('Source ID / Guardian ID')),
  ...
  Widget _buildExecutionStep(GuardianMigrationWizardState state, ThemeData theme) {
    final job = state.activeJob;
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text(
          'Please verify the details below before starting the final guardian migration database execution. This operation will commit new guardian profiles to the repository.',
        ),
        const SizedBox(height: 24),
        Card(
          elevation: 0,
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Confirm Configuration:', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                Text('• Destination School Context ID: ${job.schoolId}'),
                const SizedBox(height: 8),
                Text('• Valid guardian profiles to create: $validCount'),
                const SizedBox(height: 8),
        ...
        Row(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                final result = await ref.read(guardianMigrationWizardProvider.notifier).executeMigration();
                if (result) {
                  ref.read(guardianMigrationWizardProvider.notifier).nextStep();
                }
              },
  ...
  Widget _buildCompleteStep(GuardianMigrationWizardState state, ThemeData theme) {
    final job = state.activeJob;
    ...
    if (job.status == 'COMPLETED') {
      ...
      statusDesc = 'All valid guardian records have been created and committed successfully.';
    } else if (job.status == 'COMPLETED_WITH_ERRORS') {
      icon = Icons.warning_amber_outlined;
      color = Colors.orange;
      statusTitle = 'Migration Completed with Errors';
      statusDesc = 'Some valid guardian records were committed, but some rows failed during execution.';
    } else {
      ...
      children: [
        ...
        Table(
          ...
          children: [
            ...
            _buildTableRow('Successful Guardian Creations', '${job.successfulRows}', theme),
        ...
        Wrap(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                context.go('/guardians');
              },
              child: const Text('View Guardians'),
            ),
          ],
        ),
      ],
